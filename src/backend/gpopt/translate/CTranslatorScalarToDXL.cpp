	...
	const SubLink *sublink, const CMappingVarColId *var_colid_mapping)
{
	...
	// number of output columns generated by subselect
	const ULONG num_output_cols = query_output_dxlnode_array->Size();

	GPOS_ASSERT(0 != num_output_cols);

	// stores the column IDs of the output columns from the sublink's subselect
	ULongPtrArray *colids = GPOS_NEW(m_mp) ULongPtrArray(m_mp);

	for (ULONG col_idx = 0; col_idx < num_output_cols; col_idx++)
	{
		CDXLNode *dxl_sc_ident = (*query_output_dxlnode_array)[col_idx];
	...
	// sublink->operName represents operator name used across all OpExprs
	// within sublink->testexpr
	const CWStringConst *strOpName =
	...
	if (IsA(sublink->testexpr, BoolExpr))
	{
		...
		// multi-column scalar subquery because a BoolExpr can be an
		// AND, OR, NOT operator. AND and OR operators take 2 or more
		// values, whereas NOT will have only one value.

		// output columns of 'testexpr' are stored within outer_dxlnode
		// of type 'scalarvalueslist'.
		CDXLScalarValuesList *testexpr_output_cols =
			GPOS_NEW(m_mp) CDXLScalarValuesList(m_mp);
		outer_dxlnode = GPOS_NEW(m_mp) CDXLNode(m_mp, testexpr_output_cols);

		const BoolExpr *bool_expr = (BoolExpr *) sublink->testexpr;
	...
	else if (IsA(sublink->testexpr, OpExpr))
	{
		...
		outer_dxlnode = TranslateScalarToDXL(LHS_expr, var_colid_mapping);
	}
	else
	{
		GPOS_RAISE(gpdxl::ExmaDXL, gpdxl::ExmiQuery2DXLUnsupportedFeature,
	...
	if (ALL_SUBLINK == sublink->subLinkType)
	{
		subquery = GPOS_NEW(m_mp) CDXLScalarSubqueryAll(
			m_mp, mdids, GPOS_NEW(m_mp) CMDName(m_mp, strOpName), colids,
			testexpr_booltype);
		GPOS_DELETE(strOpName);
	}
	else
	{
