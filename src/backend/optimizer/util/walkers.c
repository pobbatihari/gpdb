...
void planner_init_plan_tree_base(plan_tree_base_prefix *base,
                                 PlannerInfo *root) {
  base->node = (Node *)root;
}
 ...
 */
bool walk_plan_node_fields(Plan *plan, bool (*walker)(), void *context) {
 ...
 */
bool walk_scan_node_fields(Scan *scan, bool (*walker)(), void *context) {
 ...
 * else they will walk common plan fields twice.
 */
bool walk_join_node_fields(Join *join, bool (*walker)(), void *context) {
 ...
 */
bool plan_tree_walker(Node *node, bool (*walker)(), void *context,
  ...
  case T_TupleSplit:
    if (walk_plan_node_fields((Plan *)node, walker, context))
      return true;
    if (expression_tree_walker((Node *)((TupleSplit *)node)->dqa_expr_lst,
                               walker, context))
      return true;
    /* Other fields are simple items and lists of simple items. */
    break;
 ...
 */
Plan *plan_tree_base_subplan_get_plan(plan_tree_base_prefix *base,
                                      SubPlan *subplan) {
  if (!base)
    return NULL;
...
/* Rewrite the plan associated with a SubPlan node in a mutator.  (This is used
 * by framework, not by users of the framework.)
 */
void plan_tree_base_subplan_put_plan(plan_tree_base_prefix *base,
                                     SubPlan *subplan, Plan *plan) {
  Assert(base);
...
static bool extract_nodes_expression_walker(Node *node,
...
static bool extract_nodes_walker(Node *node, extract_context *context) {
  ...
  if (nodeTag(node) == T_SubPlan) {
    ...
    if (extract_nodes_walker((Node *)subplan->testexpr, context))
      return true;
    if (expression_tree_walker((Node *)subplan->args, extract_nodes_walker,
                               context))
      return true;
  ...
  if (nodeTag(node) == T_TableFunctionScan && !context->descendIntoSubqueries) {
    /* Do not descend into table function subqueries */
    return false;
  }

  return plan_tree_walker(node, extract_nodes_walker, (void *)context, true);
}

/**
 * Extract nodes with specific tag.
 * Same as above, but starts off a scalar expression node rather than a
 * PlannedStmt
 *
 */
...
static bool extract_nodes_expression_walker(Node *node,
  ...
  if (nodeTag(node) == T_Query && context->descendIntoSubqueries) {
    ...
    return expression_tree_walker(
        query->havingQual, extract_nodes_expression_walker, (void *)context);
  }
...
static bool find_nodes_walker(Node *node, find_nodes_context *context) {
  ...
  if (IsA(node, Query)) {
    ...
    return query_tree_walker((Query *)node, find_nodes_walker, (void *)context,
                             0 /* flags */);
  }
...
static bool check_collation_walker(Node *node,
...
static void check_collation_in_list(List *colllist,
                                    check_collation_context *context) {
  ListCell *lc;
  foreach (lc, colllist) {
...
static bool check_collation_walker(Node *node,
  ...
  case T_DMLActionExpr:
    ...
    if ((InvalidOid != collation && DEFAULT_COLLATION_OID != collation) ||
        (InvalidOid != inputCollation &&
         DEFAULT_COLLATION_OID != inputCollation)) {
      context->foundNonDefaultCollation = 1;
    }
    break;
  ...
  case T_RangeTblFunction:
    check_collation_in_list(((RangeTblFunction *)node)->funccolcollations,
                            context);
    break;
  case T_CommonTableExpr:
    check_collation_in_list(((CommonTableExpr *)node)->ctecolcollations,
                            context);
    break;
  case T_SetOperationStmt:
  ...
  if (context->foundNonDefaultCollation == 1) {
    ...
    return expression_tree_walker(node, check_collation_walker,
                                  (void *)context);
  }
}
